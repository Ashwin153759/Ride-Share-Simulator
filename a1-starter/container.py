...
class PriorityQueue(Container):
    ...
    def add(self, item: object) -> None:
        ...
        """
        starting_len = len(self._items)

        # loop through queue to see if it's less than any other elements
        # if so insert in proper spot
        for i in range(len(self._items)):
            if item.__lt__(self._items[i]):
                self._items.insert(i, item)
                break
        # if the item has the least priority (greater than
        # everything in the list), add to end of list
        if len(self._items) == starting_len:
